 ...
 */
public class BeeeyeWindowsSafeLibController extends Controller {

    private static final WindowsSafeLib dao = new WindowsSafeLib().dao();
    private final String tableName = "beeeye_baseline_win";

    public void get() throws JSONException {
        String ids = getPara();
        if (ids != null) {
            WindowsSafeLib windowsSafeLib = dao.findById(ids);
            String[] Names = windowsSafeLib._getAttrNames();
            JSONObject obj = new JSONObject();
            for (String param : Names) {
                Object object = windowsSafeLib.get(param);
            ...
            if (json == null || json.isEmpty()) {
                List<WindowsSafeLib> list = dao.find("select * from " + tableName);
                JSONArray postList = new JSONArray();
                for (WindowsSafeLib item : list) {
                ...
                Page<WindowsSafeLib> paginate = dao.paginate(pageNumber, pageSize, "select *", " from " + tableName);
                List<WindowsSafeLib> list = paginate.getList();
                JSONArray postList = new JSONArray();
                for (WindowsSafeLib item : list) {
                    String[] Names = item._getAttrNames();
                    JSONObject obj = new JSONObject();
    ...
    public void put() {
        ...
        if (ids != null) {
            ...
            WindowsSafeLib bean = getBean(WindowsSafeLib.class);
            bean.setIds(ids);
